        ...
        if( timeleft <= 0 ) // time to change to another behaviour
            switch( state ) {
                case S_PUSH:
                    ...
                    break;
                case S_BACKUP_LONG: 
                    //std::cout << "In state: BACKUP_LONG - Light Intensity: " << GetLightIntensity() << std::endl;
                    if( currentLightIntensity > lightCTRL->GetLargeLight() )
...
int main( int argc, char* argv[] ){
    ...
    while( !world.RequestShutdown() ) {
        ...
        world.Step( timeStep, robots, boxes, goals );
